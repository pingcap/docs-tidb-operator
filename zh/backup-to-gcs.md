...
* `.spec.cleanPolicy`：备份集群后删除备份 CR 时的备份文件清理策略。目前支持三种清理策略：
    ...
    `OnFailure`：仅在备份中失败时，删除备份 CR 时会删除备份出的文件
    
    默认时不填写该字段与填写上述字段以外的字段均会保留备份出的文件。值得注意的是，在 v1.1.2 以及之前版本不存在该字段，且默认在删除 CR 的同时删除备份的文件。若 v1.1.3 及之后版本的用户希望保持该行为，需要设置该字段为 `Delete`。
