...
3. 连接到 TiDB 集群：
    ...
    > **注意：**
    >
    > * [MySQL 8.0 默认认证插件从 `mysql_native_password` 更新为 `caching_sha2_password`](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html#sysvar_default_authentication_plugin)，因此如果使用 MySQL 8.0 客户端访问 TiDB 服务并且用户账户有配置密码，需要显示指定 `--default-auth=mysql_native_password` 参数。
