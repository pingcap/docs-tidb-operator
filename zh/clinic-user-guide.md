...
1. 获取你要部署的 Diag chart 中的 `values-diag-collector.yaml` 文件。
    ...
    > **注意：**
    >
    > `${chart_version}` 在后续文档中代表 chart 版本，例如 `v1.2.1`，可以通过 `helm search repo -l diag` 查看当前支持的版本。
...
1. 下载 Diag chart。
    ...
    将 `diag-v1.2.1.tgz` 文件拷贝到服务器上并解压到当前目录：
...
2. 下载 Diag 运行所需的 Docker 镜像。
    ...
    TiDB Operator 用到的 Docker 镜像为 `pingcap/diag:v1.2.1`，通过下面的命令将镜像下载下来：
