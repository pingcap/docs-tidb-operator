...
EBS 卷快照恢复阶段包含以下阶段，详细见 [基于 EBS 卷快照的备份恢复功能架构](volume-snapshot-backup-restore.md)。
...
> **注意：**
>
> 因为卷快照是崩溃一致性 （Crash Consistency），EBS 卷快照恢复阶段 TiKV 启动阶段，需要数据自检和自动修复，经测试，耗时在 30 分钟以内，如使用高性能盘恢复，如提升 IOPS 和带宽, 额外耗时可缩短到 5 分钟内。使用高性能盘恢复请参考[恢复时间太长（大于 2 小时）](backup-restore-faq.md#恢复时间太长大于-2-小时)
>
