...
为了优化各个组件的启动脚本，并且不引起升级 Operator 后导致集群滚动重启，集群配置增加了 `spec.startScriptVersion` 字段用于选择各个组件的不同版本的启动脚本。

目前支持的启动脚本的版本有：

* `v1`：默认值，最初版本的启动脚本

* `v2`：(Operator 版本 >= 1.4.0) 增加了以下的优化：

    * 使用 `dig` 命令替换 `nslookup` 命令来解析 DNS。
    * 所有组件都支持[诊断模式](tips.md#诊断模式)。

新部署的集群建议配置 `spec.startScriptVersion` 为最新的版本。

> **警告：**
>
> 修改已经部署的集群的 `startScriptVersion` 会导致集群滚动重启。
