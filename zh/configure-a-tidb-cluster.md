...
* 为 TiDB 节点设置所在的 Node 节点的拓扑信息(仅支持版本 >=v6.3.0 的集群)

    TiDB Operator 会自动为 TiDB 获取其所在节点的拓扑信息，并调用 TiDB Server 的对应接口将这些信息设置为 TiDB 的 Labels, 这样 TiDB 可以根据这些 Labels 将 [Follower Read](https://docs.pingcap.com/zh/tidb/dev/follower-read) 的请求发送至正确的副本。
