...
## 什么是跨多个 Kubernetes 集群部署 TiDB 集群

跨多个 Kubernetes 集群部署 TiDB 集群是指在多个互相联通的 Kubernetes 集群部署数据联通, 可以在集群间容灾, 扩缩容的 TiDB 集群. 所谓 Kubernetes 集群互相连通, 在此文档中指部署的目标 Kubernetes 集群都处于同一网络环境下(如同一 VPC 下), POD IP 在任意集群内和集群间可互相访问, FQDN 记录是集群内和集群间均可被解析的. 满足上述条件的 Kubernetes 集群可以进行 TiDB 集群部署.
...
- 部署新的开启跨多个 Kubernetes 集群的 TiDB 集群, 允许在其他 Kubernetes 集群上部署开启此功能的新集群加入同样开启此功能的集群
...
## 已有数据集群开启跨多个 Kubernetes 集群能力并作为 TiDB 集群的初始集群
