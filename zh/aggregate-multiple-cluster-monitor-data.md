...
1. 为每个 TidbMonitor 配置一个 Thanos Sidecar 容器。
    ...
    ```
    kubectl -n ${namespace} apply -f https://raw.githubusercontent.com/pingcap/tidb-operator/v1.4.2/examples/monitor-with-thanos/tidb-monitor.yaml
    ```
...
2. 部署 Thanos Query 组件。
    ...
    1. 下载 Thanos Query 的部署文件 `thanos-query.yaml`：
        ...
        ```
        curl -sl -O https://raw.githubusercontent.com/pingcap/tidb-operator/v1.4.2/examples/monitor-with-thanos/thanos-query.yaml
        ```
