...
aliases: ['/docs-cn/tidb-in-kubernetes/stable/deploy-tidb-operator/','/docs-cn/tidb-in-kubernetes/v1.1/deploy-tidb-operator/','/docs-cn/stable/tidb-in-kubernetes/deploy/tidb-operator/','/docs-cn/v3.0/how-to/deploy/tidb-operator/']
---
...
1. 获取你要部署的 `tidb-operator` chart 中的 `values.yaml` 文件：
    ...
    > **注意：**
    >
    > `${chart_version}` 在后续文档中代表 chart 版本，例如 `v1.1.7`，可以通过 `helm search -l tidb-operator` 查看当前支持的版本。
...
1. 下载 `tidb-operator` chart
    ...
    将 `tidb-operator-v1.1.7.tgz` 文件拷贝到服务器上并解压到当前目录：
