...
Kubernetes [基于角色的访问控制 (RBAC)](https://kubernetes.io/docs/reference/access-authn-authz/rbac/) 功能是通过 Role 或者 ClusterRole 来进行管理的，并通过 RoleBinding 或 ClusterRoleBinding 将其权限赋予一个或者一组用户。默认配置下，安装的 TiDB Operator 是 cluster-scoped，能管理 Kubernetes 集群内所有 TiDB 集群，通过 ClusterRole 授予其需要的权限。要查看为 TiDB Operator 创建的 ClusterRole，请使用以下命令：
...
| statefulsets.apps.pingcap.com                 | -                 | -              | [*]                                              | AdvancedStatefulSet=true 时，需要操作此资源，详细信息可以参考[文档](advanced-statefulset.md) |
...
1. 获取你要部署的 `tidb-operator` chart 中的 `values.yaml` 文件：
    ...
    > **注意：**
    >
    > `${chart_version}` 在后续文档中代表 chart 版本，例如 `v1.1.13`，可以通过 `helm search repo -l tidb-operator` 查看当前支持的版本。
...
1. 下载 `tidb-operator` chart
    ...
    将 `tidb-operator-v1.1.13.tgz` 文件拷贝到服务器上并解压到当前目录：
