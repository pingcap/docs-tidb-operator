...
2. 更新 Kubernetes 的 CustomResourceDefinition (CRD)。关于 CRD 的更多信息，请参阅 [CustomResourceDefinition](https://kubernetes.io/docs/tasks/access-kubernetes-api/custom-resources/custom-resource-definitions/)。
    ...
    本文以 TiDB Operator v1.6.0-beta.1 为例，你需要替换 `${operator_version}` 为你要升级到的 TiDB Operator 版本。
...
4. 修改 `${HOME}/tidb-operator/v1.6.0-beta.1/values-tidb-operator.yaml` 中 `operatorImage` 镜像版本为要升级到的版本。

5. 如果你在旧版本 `values.yaml` 中设置了自定义配置，将自定义配置合并到 `${HOME}/tidb-operator/v1.6.0-beta.1/values-tidb-operator.yaml` 中。
...
7. Pod 全部正常启动之后，运行以下命令确认 TiDB Operator 镜像版本：
    ...
    如果输出类似下方的结果，则表示升级成功。其中，`v1.6.0-beta.1` 表示已升级到的版本号。
...
3. 修改 `${HOME}/tidb-operator/v1.6.0-beta.1/values-tidb-operator.yaml` 中 `operatorImage` 镜像版本为要升级到的版本。

4. 如果你在旧版本 `values.yaml` 中设置了自定义配置，将自定义配置合并到 `${HOME}/tidb-operator/v1.6.0-beta.1/values-tidb-operator.yaml` 中。
