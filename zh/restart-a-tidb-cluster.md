...
如果有日志备份任务正在运行，TiDB Operator 会在删除 Pod 之前尝试对备份日志强制落盘。你可以通过在 `TiDBCluster` CR 中添加 annotation `tidb.pingcap.com/tikv-restart-without-flush-log-backup` 并设置任意值来禁用此功能。