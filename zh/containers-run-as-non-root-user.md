...
对于按照 Custom Resource (CR) 生成的容器，可以在任意一种 CR（例如 `PDGroup`、`TiDBGroup`、`TiKVGroup`、`TiFlashGroup`、`TiCDCGroup`、`Backup`、`CompactBackup`、`Restore`）中配置安全上下文 (`securityContext`)。
...
- 对于 `Backup`、`CompactBackup`、`Restore` 等 CR，可以在 `spec` 中配置 `podSecurityContext`。配置 `Backup` CR 的示例如下：
