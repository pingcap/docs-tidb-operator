...
2. 部署 TiDB 集群：
    ...
    ``` shell
    kubectl apply -f https://raw.githubusercontent.com/pingcap/tidb-operator/{{{ .tidb_operator_version }}}/examples/basic/tidb-cluster.yaml -n demo
    ```

3. 部署 TiDB 集群监控：

    ``` shell
    kubectl apply -f https://raw.githubusercontent.com/pingcap/tidb-operator/{{{ .tidb_operator_version }}}/examples/basic/tidb-monitor.yaml -n demo
    ```
