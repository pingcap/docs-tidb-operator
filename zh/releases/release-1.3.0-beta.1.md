...
- 由于 [#4151](https://github.com/pingcap/tidb-operator/pull/4151) 的变更，1.3.0-beta.1 后 Operator 会默认设置各组件的 `baseImage` 字段。如果你使用了各组件的 `image` 字段而不是 `baseImage` 字段来设置，那么直接升级 1.3.0-beta.1 及以后的 TiDB Operator 可能会因为使用错误的镜像导致 TiDB 集群滚动重建甚至无法正常运行。你必须按照以下操作来升级 TiDB Operator：
