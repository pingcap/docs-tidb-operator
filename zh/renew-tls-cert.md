...
本文以更新和替换 TiDB 集群中 PD、TiKV、TiDB 组件间的证书为例，介绍在证书过期之前，如何更新和替换相应组件的证书。
...
5. 参考[滚动重启 TiDB 集群](https://docs.pingcap.com/zh/tidb-in-kubernetes/stable/restart-a-tidb-cluster) 对需要加载组合 CA 证书的组件进行滚动重启。
...
3. 参考[滚动重启 TiDB 集群](https://docs.pingcap.com/zh/tidb-in-kubernetes/stable/restart-a-tidb-cluster) 对需要加载新证书的组件进行滚动重启。

    滚动重启完成后，各组件将使用新的证书进行 TLS 通信。如有参考[更新和替换 CA 证书](#更新和替换-ca-证书)并使各组件加载了组合 CA 证书，则其仍能接受由原 CA 证书签发的证书。

### 可选：移除组合 CA 证书中的原 CA 证书

若同时参考[更新和替换 CA 证书](#更新和替换-ca-证书)与[更新和替换组件间证书](#更新和替换组件间证书)更新与替换了组合 CA 证书与 Server 端、Client 端组件，且计划移除原 CA 证书（如原 CA 证书已过期或原 CA 证书的密钥被盗），则可按如下步骤移除原 CA 证书。
...
2. 参考[滚动重启 TiDB 集群](https://docs.pingcap.com/zh/tidb-in-kubernetes/stable/restart-a-tidb-cluster) 对需要加载新证书的组件进行滚动重启。
