...
aliases: ['/docs-cn/tidb-in-kubernetes/stable/restore-from-gcs/','/docs-cn/tidb-in-kubernetes/v1.1/restore-from-gcs/','/docs-cn/stable/tidb-in-kubernetes/maintain/backup-and-restore/restore-gcs/','/docs-cn/dev/tidb-in-kubernetes/maintain/backup-and-restore/restore-gcs/']
---
...
本文使用的恢复方式基于 TiDB Operator v1.1 及以上的 CustomResourceDefinition (CRD) 实现，底层通过使用 [TiDB Lightning TiDB-backend](https://pingcap.com/docs/stable/how-to/get-started/tidb-lightning/#tidb-lightning-tutorial) 来恢复数据。

目前，TiDB Lightning 支持 `Importer-backend`, `Local-backend` 及 `TiDB-backend` 三种后端，对于这三种后端的区别和选择，可以参考[TiDB Lightning 文档](https://docs.pingcap.com/zh/tidb/stable/tidb-lightning-backends)。如果要使用 `Importer-backend` 或者 `Local-backend` 导入数据，请参考[使用 TiDB Lightning 导入集群数据](restore-data-using-tidb-lightning.md)。
