...
* [全量备份](#全量备份)（定时执行或 Ad-hoc）：使用 [`Dumpling`](https://pingcap.com/docs-cn/stable/dumpling-overview/) 获取集群的逻辑备份；
...
目前，Kubernetes 上的 TiDB 集群只对 `Dumpling` 获取的全量备份数据提供自动化的数据恢复操作。恢复 `TiDB-Binlog` 获取的增量数据需要手动进行。

## 全量备份

全量备份使用 `Dumpling` 来获取 TiDB 集群的逻辑备份数据。全量备份任务会创建一个 PVC ([PersistentVolumeClaim](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#persistentvolumeclaims)) 来存储数据。
