...
+ `compactBackupTemplate`: 压缩日志备份的配置模板，字段和 CompactBackup CR 中的 `spec` 一样，详细介绍可参考 [ComapctBackup CR 字段介绍](#compactbackup-cr-字段介绍 )，压缩日志备份随 `backupSchedule` 创建、删除，日志备份名称在 `status.logBackup` 中保存。压缩日志备份的存储设置应与同一 `backupSchedule` 中的 `logBackupTemplate` 相同
