...
* `.spec.backupMode`：指定 Backup 的模式，默认为 `snapshot`。目前支持以下两种种类型：
    * `snapshot`：基于 KV 层的快照备份。
    * `log`：从 KV 层备份实时数据变更日志数据。

* `.spec.logSubcommand`：指定日志备份任务的子命令，用于控制日志备份任务的状态。该字段支持以下五个选项：
...
* `.spec.tableFilter`：备份时指定让BR 备份符合 [table-filter 规则](https://docs.pingcap.com/zh/tidb/stable/table-filter/)的表。默认情况下该字段可以不用配置。当不配置时，BR 会备份除系统库以外的所有数据库。
