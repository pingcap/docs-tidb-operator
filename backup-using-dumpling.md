---
title: 使用 Dumpling 备份 TiDB 集群数据
summary: 介绍如何使用 Dumpling 备份 Kubernetes 上的 TiDB 集群数据到远程存储。
aliases: ['/docs-cn/tidb-in-kubernetes/dev/backup-to-gcs/', '/docs-cn/tidb-in-kubernetes/dev/backup-to-s3/']
---

# 使用 Dumpling 备份 TiDB 集群数据

本文档详细描述了如何使用 Dumpling 将 Kubernetes 上的 TiDB 集群数据备份到远程存储。目前 Dumpling 支持备份集群数据到 [Google Cloud Storage (GCS)](https://cloud.google.com/storage/docs/) 和 [Amazon S3](https://aws.amazon.com/s3/)。本文档中的“备份”，均是指全量备份（Ad-hoc 全量备份和定时全量备份），底层通过使用 [`Dumpling`](https://docs.pingcap.com/zh/tidb/stable/dumpling-overview) 获取集群的逻辑备份，然后再将备份数据上传到远程存储。

本文使用的备份方式基于 TiDB Operator 新版（v1.1 及以上）的 CustomResourceDefinition (CRD) 实现。下面分别介绍备份集群数据到 GCS 和兼容 S3 的存储的操作方式。

## 备份 TiDB 集群数据到 GCS

以下示例将备份 TiDB 集群数据到 [Google Cloud Storage (GCS)](https://cloud.google.com/storage/docs/)。
