---
title: 使用 BR 工具备份 TiDB 集群数据
summary: 介绍如何使用 BR 工具备份 TiDB 集群数据到远程存储。
aliases: ['/docs-cn/tidb-in-kubernetes/dev/backup-to-gcs-using-br/', '/docs-cn/tidb-in-kubernetes/dev/backup-to-aws-s3-using-br/']
---

# 使用 BR 工具备份 TiDB 集群数据

本文档详细描述了如何使用 BR 将 Kubernetes 上 TiDB 集群的数据备份到远程存储。目前 BR 支持备份集群数据到 [Google Cloud Storage (GCS)](https://cloud.google.com/storage/docs/) 和 [Amazon S3](https://aws.amazon.com/s3/)。[`BR`](https://docs.pingcap.com/zh/tidb/stable/backup-and-restore-tool) 会在底层获取集群的逻辑备份，然后再将备份数据上传到远程存储。
...
2. 存储访问授权
    ...
    如果使用 Amazon S3 来备份恢复集群，可以使用三种权限授予方式授予权限，参考 [AWS 账号授权](grant-permissions-to-remote-storage.md#AWS-账号授权) 授权访问兼容 S3 的远程存储。
