...
aliases: ['/docs/tidb-in-kubernetes/dev/deploy-tidb-operator/','/tidb-in-kubernetes/dev/deploy-on-alibaba-cloud','/docs/tidb-in-kubernetes/dev/deploy-on-alibaba-cloud/','/tidb-in-kubernetes/v1.6/deploy-on-alibaba-cloud','/tidb-in-kubernetes/stable/deploy-on-alibaba-cloud']
---
...
```shell
wget https://raw.githubusercontent.com/pingcap/tidb-operator/{{{ .tidb_operator_version }}}/manifests/crd.yaml
...
1. Get the `values.yaml` file of the `tidb-operator` chart you want to deploy:
    ...
    > **Note:**
    >
    > `${chart_version}` represents the chart version of TiDB Operator. For example, `{{{ .tidb_operator_version }}}`. You can view the currently supported versions by running the `helm search repo -l tidb-operator` command.
...
2. Download the Docker images used by TiDB Operator
    ...
    ```shell
    pingcap/tidb-operator:{{{ .tidb_operator_version }}}
    pingcap/tidb-backup-manager:{{{ .tidb_operator_version }}}
