...
```shell
kubectl apply -f https://raw.githubusercontent.com/pingcap/tidb-operator/v1.1.3/manifests/crd.yaml
```
...
```shell
wget https://raw.githubusercontent.com/pingcap/tidb-operator/v1.1.3/manifests/crd.yaml
...
1. Get the `values.yaml` file of the `tidb-operator` chart you want to install.
    ...
    > **Note:**
    >
    > `${chart_version}` represents the chart version of TiDB Operator. For example, `v1.1.3`. You can view the currently supported versions by running the `helm search -l tidb-operator` command.
...
1. Download the `tidb-operator` chart
    ...
    ```shell
    wget http://charts.pingcap.org/tidb-operator-v1.1.3.tgz
    ```

    Copy the `tidb-operator-v1.1.3.tgz` file to the target server and extract it to the current directory:

    {{< copyable "shell-regular" >}}

    ```shell
    tar zxvf tidb-operator.v1.1.3.tgz
    ```
...
2. Download the Docker images used by TiDB Operator
    ...
    docker save -o tidb-operator-v1.1.3.tar pingcap/tidb-operator:v1.1.3
    docker save -o tidb-backup-manager-v1.1.3.tar pingcap/tidb-backup-manager:v1.1.3
    ...
    ```shell
    docker load -i tidb-operator-v1.1.3.tar
    docker load -i tidb-backup-manager-v1.1.3.tar
