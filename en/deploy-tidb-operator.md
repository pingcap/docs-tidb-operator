...
```shell
wget https://raw.githubusercontent.com/pingcap/tidb-operator/v1.1.2/manifests/crd.yaml
...
1. Get the `values.yaml` file of the `tidb-operator` chart you want to install.
    ...
    > **Note:**
    >
    > `${chart_version}` represents the chart version of TiDB Operator. For example, `v1.1.2`. You can view the currently supported versions by running the `helm search -l tidb-operator` command.
...
1. Download the `tidb-operator` chart
    ...
    Copy the `tidb-operator-v1.1.2.tgz` file to the target server and extract it to the current directory:
...
2. Download the Docker images used by TiDB Operator
    ...
    ```shell
    pingcap/tidb-operator:v1.1.2
    pingcap/tidb-backup-manager:v1.1.2
    ...
    docker save -o tidb-operator-v1.1.2.tar pingcap/tidb-operator:v1.1.2
    docker save -o tidb-backup-manager-v1.1.2.tar pingcap/tidb-backup-manager:v1.1.2
    ...
    ```shell
    docker load -i tidb-operator-v1.1.2.tar
    docker load -i tidb-backup-manager-v1.1.2.tar
