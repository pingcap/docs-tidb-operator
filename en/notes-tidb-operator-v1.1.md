...
- If the TiDB cluster version < v3.1, refer to [Restore data using Lightning](restore-from-s3.md).
- If the TiDB cluster version >= v3.1, refer to [Restore data using BR](restore-from-aws-s3-using-br.md).

> **Note:**
>
> Currently, with Backup CR, you can restore data only from S3 and GCS using Lightning, and restore data from S3 using BR. If you need to restore the backup data from local Persistent Volume Claim (PVC), you cannot switch to the CR management.
