...
If you have the following backup needs, you can use BR to make an [ad-hoc backup](#ad-hoc-full-backup-to-gcs) or [scheduled full backup](#scheduled-full-backup-to-gcs) of the TiDB cluster data to GCS.
...
1. Create the `Backup` CR to back up cluster data to GCS as described below:
    ...
    - Some parameters in `spec.gcs` can be ignored, such as `location`, `objectAcl`, and `storageClass`. For more information about GCS configuration, refer to [GCS fields](backup-restore-overview.md#gcs-fields).
