...
1. Decide on instance types.
    ...
    > **Note:**
    >
    > * Check the default TiDB Operator version of the current script in the `tidb_operator_version` of the `variables.tf` file. If the default version is not your desired one, configure `tidb_operator_version` in `terraform.tfvars`.
