...
1. Update [CRD (Custom Resource Definition)](https://kubernetes.io/docs/tasks/access-kubernetes-api/custom-resources/custom-resource-definitions/):
    ...
    > **Note:**
    >
    > The `${version}` in this document represents the version of TiDB Operator, such as `v1.1.11`. You can check the currently supported version using the `helm search repo -l tidb-operator` command.
