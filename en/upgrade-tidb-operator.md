...
2. Update [CustomResourceDefinition](https://kubernetes.io/docs/tasks/access-kubernetes-api/custom-resources/custom-resource-definitions/) (CRD) for Kubernetes:
    ...
    * If your Kubernetes version < v1.16:
    ...
    This document takes TiDB v1.5.5 as an example. You can replace `${operator_version}` with the specific version you want to upgrade to.
...
3. Get the `values.yaml` file of the `tidb-operator` chart:
    ...
    ```bash
    mkdir -p ${HOME}/tidb-operator/v1.5.5 && \
    helm inspect values pingcap/tidb-operator --version=v1.5.5 > ${HOME}/tidb-operator/v1.5.5/values-tidb-operator.yaml
    ```

4. In the `${HOME}/tidb-operator/v1.5.5/values-tidb-operator.yaml` file, modify the `operatorImage` version to the new TiDB Operator version.

5. If you have added customized configuration in the old `values.yaml` file, merge your customized configuration to the `${HOME}/tidb-operator/v1.5.5/values-tidb-operator.yaml` file.
...
6. Perform upgrade:
    ...
    ```bash
    helm upgrade tidb-operator pingcap/tidb-operator --version=v1.5.5 -f ${HOME}/tidb-operator/v1.5.5/values-tidb-operator.yaml -n tidb-admin
    ```
...
7. After all the Pods start normally, check the image of TiDB Operator:
    ...
    If you see a similar output as follows, TiDB Operator is successfully upgraded. `v1.5.5` represents the TiDB Operator version you have upgraded to.

    ```
    image: pingcap/tidb-operator:v1.5.5
    image: docker.io/pingcap/tidb-operator:v1.5.5
    image: pingcap/tidb-operator:v1.5.5
    image: docker.io/pingcap/tidb-operator:v1.5.5
    ```
...
1. Download the files and images required for the upgrade using a machine with Internet access:
    ...
    3. Download the Docker images required for the new TiDB Operator version:
        ...
        docker save -o tidb-operator-v1.5.5.tar pingcap/tidb-operator:v1.5.5
        docker save -o tidb-backup-manager-v1.5.5.tar pingcap/tidb-backup-manager:v1.5.5
        ```
...
2. Upload the downloaded files and images to the server where TiDB Operator is deployed, and install the new TiDB Operator version:
    ...
    3. Unpack the `tidb-operator` chart package file, and copy the `values.yaml` file to the directory of the new TiDB Operator:
        ...
        mkdir -p ${HOME}/tidb-operator/v1.5.5 && \
        cp tidb-operator/values.yaml ${HOME}/tidb-operator/v1.5.5/values-tidb-operator.yaml
        ```
    ...
    4. Install the Docker images on the server:
        ...
        ```bash
        docker load -i tidb-operator-v1.5.5.tar && \
        docker load -i tidb-backup-manager-v1.5.5.tar
        ```

3. In the `${HOME}/tidb-operator/v1.5.5/values-tidb-operator.yaml` file, modify the `operatorImage` version to the new TiDB Operator version.

4. If you have added customized configuration in the old `values.yaml` file, merge your customized configuration to the `${HOME}/tidb-operator/v1.5.5/values-tidb-operator.yaml` file.
...
5. Perform upgrade:
    ...
    ```bash
    helm upgrade tidb-operator ./tidb-operator --version=v1.5.5 -f ${HOME}/tidb-operator/v1.5.5/values-tidb-operator.yaml
    ```
