...
> **Note:**
>
> [The default authentication plugin of MySQL 8.0 is updated from `mysql_native_password` to `caching_sha2_password`](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html#sysvar_default_authentication_plugin), so if you use MySQL client from MySQL 8.0 to access the TiDB service, the TiDB version < v4.0.7, and the user account has a password, you need to explicitly specify the `--default-auth=mysql_native_password` parameter.
