...
This document introduces how to renew and replace certificates of the corresponding components before certificates expire, taking TLS certificates between PD, TiKV and TiDB components in the TiDB cluster for example.
...
5. Perform the rolling restart to components that need to load the combined CA certificate, refer to [perform a rolling restart to the TiDB cluster](restart-a-tidb-cluster.md).

    After the completion of the rolling restart, based on the combined CA certificate, each component can accept the certificate issued by the original CA certificate and the new CA certificate at the same time.
...
1. Generate new server-side and client-side certificates based on the original configuration information of each component.
    ...
    > **Note:**
    >
    > - In the above command, assume that you refer to steps in [renew and replace the TLS Certificate](#renew-and-replace-the-tls-certificate) to back up the new CA certificate as `ca.new.pem` and key as `ca-key.new.pem`. If you do not renew the CA certificate and the key, modify the corresponding parameters in the command to `ca.pem` and `ca-key.pem`.
...
3. Perform the rolling restart to components that need to load the combined CA certificate, refer to [perform a rolling restart to the TiDB cluster](restart-a-tidb-cluster.md).
...
If you renew and replace the combined CA certificate, server-side and client-side certificates between components by referring to [Renew and replace the CA certificate](#renew-and-replace-the-ca-certificate) and [Renew and replace certificates between components](#renew-and-replace-certificates-between-components), you can remove the original CA certificate as follows.
...
When using `cert-manager` to issue the certificate, the `cert-manager` can automatically update the certificate before it expires by specifying the `spec.renewBefore` of the `Certificate` resource.

Although `cert-manager` can automatically renew the CA certificate and the corresponding Kubernetes Secret objects, it currently does not support merging the old and new CA certificates into a combined CA certificate to accept certificates issued by the new and old CA certificates at the same time. Therefore, when renewing and replacing the CA certificate, the issue that TLS between cluster components can not authenticate each other might occur.

> **Warning:**
>
> Because the certificates issued by the new and old CAs can not be accepted between components at the same time, renewing and replacing certificates needs to rebuild the Pods of some components. This might cause some requests to access the TiDB cluster to fail.
...
1. The `cert-manager` automatically renews the CA certificate and the Kubernetes Secret object `${cluster_name}-ca-secret` before the certificate expires.
    ...
    To manually renew the CA certificate, you can directly delete the corresponding Kubernetes Secret objects and trigger the `cert-manager` to regenerate the CA certificate.
