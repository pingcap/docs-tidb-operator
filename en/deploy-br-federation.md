...
```shell
kubectl create -f https://raw.githubusercontent.com/pingcap/tidb-operator/v1.5.3/manifests/federation-crd.yaml
```
...
4. Get the `values.yaml` file of the desired `br-federation` chart for deployment.
    ...
    helm inspect values pingcap/br-federation --version=v1.5.3 > ${HOME}/br-federation/values.yaml
    ```
...
6. Deploy the BR Federation.
    ...
    ```shell
    helm install --namespace br-fed-admin br-federation pingcap/br-federation --version v1.5.3 -f ${HOME}/br-federation/values.yaml && \
