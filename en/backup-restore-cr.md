...
* `.spec.toolImage`: the tool image used by `Backup`. TiDB Operator supports this configuration item starting from v1.1.9.
    ...
    - When using Dumpling for backup, you can specify the Dumpling version in this field.
        ...
        - If the field is not specified, the Dumpling version specified in `TOOLKIT_VERSION` of the [Backup Manager Dockerfile](https://github.com/pingcap/tidb-operator/blob/v1.3.10/images/tidb-backup-manager/Dockerfile) is used for backup by default.
