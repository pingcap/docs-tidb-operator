...
* `compactBackupTemplate`: the configuration of the compact backup. Specifies the configuration related to the cluster and remote storage of the compact backup, which is the same as the `spec` configuration of [the `CompactBackup` CR](#backup-cr-fields). The compact backup is created and deleted along with `backupSchedule` and recycled according to `.spec.maxReservedTime`. The storage setting of the compact backup should be as same as the `logBackupTemplate` in the same `backupSchedule`
...
* The unique configuration items of `backupSchedule` are as follows:
    ...
    * 1. If more than `compactInterval` time has passed since the last `compactExecutionTs`.
    * 2. If the log backup checkpoint advanced the `compactProgress` more than `compactInterval` time.
