...
- Can't set TiKV configuration `resolved-ts.enable: false` or `raftstore.report-min-resolved-ts-interval: 0s`, or it will lead to backup failure
- Can't set PD configuration `pd-server.min-resolved-ts-persistence-interval: 0s`, or it will lead to backup failure
