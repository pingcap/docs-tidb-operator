...
The following process uses `/mnt/disks` as the discovery directory and `local-storage` as the StorageClass name to create a PV. If you need to use a different data disk or StorageClass for monitoring, backup, etc., refer to the following [example](#disk-mount-examples) for configuration.
...
2. Deploy `local-volume-provisioner`.
    ...
    > - If no mount point is found in the directory, no PV is created and the output of `kubectl get pv | grep local-storage` is empty.
