...
* `.spec.toolImage`: The tool image used by `Backup`.
    - When using BR for backup, you can specify the BR version in this field.
        ...
        - If the value contains tag, like `.spec.toolImage: pingcap/br:v5.0.1`, specified image is used for backup.
        - If the value does not contains tag, like `.spec.toolImage: your_image`, image `your_image:${tikv_version}` is used for backup.
