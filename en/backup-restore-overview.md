...
* `.spec.toolImage`: The tool image used by `Backup`.
    - When using BR for backup, you can specify the BR version in this field.
        ...
        - If the BR version is specified in this field, such as `.spec.toolImage: pingcap/br:v5.0.4`, the image of the specified version is used for backup.
    ...
    - When using Dumpling for backup, you can specify the Dumpling version in this field. For example, `spec.toolImage: pingcap/dumpling:v5.0.4`. If not specified, the Dumpling version specified in `TOOLKIT_V40` of the [Backup Manager Dockerfile](https://github.com/pingcap/tidb-operator/blob/release-1.1/images/tidb-backup-manager/Dockerfile) is used for backup by default.
