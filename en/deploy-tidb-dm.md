...
To configure the DM deployment, you need to configure the `DMCluster` Custom Resource (CR). For the complete configurations of the `DMCluster` CR, refer to the [DMCluster example](https://github.com/pingcap/tidb-operator/blob/v1.5.5/examples/dm/dm-cluster.yaml) and [API documentation](https://github.com/pingcap/tidb-operator/blob/v1.5.5/docs/api-references/docs.md#dmcluster). Note that you need to choose the example and API of the current TiDB Operator version.
...
- `spec.<master/worker>.version`: the format is `imageTag`, such as `v7.5.5`.
...
1. Deploy a DM cluster requires the following Docker image (assuming the version of the DM cluster is v7.5.5):
...
2. To download the image, execute the following command:
    ...
    ```shell
    docker pull pingcap/dm:v7.5.5
    docker save -o dm-v7.5.5.tar pingcap/dm:v7.5.5
    ```
